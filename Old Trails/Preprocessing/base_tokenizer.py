...
class BaseTokenizer:
    ...
    def encode_sentences(self, sentences, boundries=None, out_length=None):
        ...
        for sent in sentences:
            ...
            if boundries:
                encoded = (
                    [self.token_to_id(boundries[0])]
                    + encoded
                    + [self.token_to_id(boundries[1])]
                )
    ...
    def calculate_compression_factor(self, text, normalized=True):
        ...
        for word in words:
            ...
            factor += len(word) + 1 if self.unk_token in tokenized else len(tokenized)
        if normalized:
            normalized_factor = factor / (sum(len(word) + 1 for word in words))
